...
contract NFTGenetics is ERC721Enumerable {
    ...
    function mint() public payable {
        ...
        uint256 newDna = uint256(keccak256(abi.encodePacked(msg.sender, block.timestamp)));
    ...
    function breed(uint256 _nft1Id, uint256 _nft2Id) public {
        require(_nft1Id < nfts.length && _nft2Id < nfts.length, "Invalid NFT IDs");
        NFT storage nft1 = nfts[_nft1Id];
        NFT storage nft2 = nfts[_nft2Id];

        require(nft1.breedingCount < 5 && nft2.breedingCount < 5, "NFT has reached breeding limit");

        uint256 childDna = mixGenes(nft1.dna, nft2.dna);

        NFT memory childNFT = NFT({
            dna: childDna,
            breedingCount: 0
        });
        nfts.push(childNFT);
    ...
    function mixGenes(uint256 _genes1, uint256 _genes2) internal view returns (uint256) {
    ...
    function getGenomeByTokenId(uint256 _tokenId) external view returns (uint256) {
